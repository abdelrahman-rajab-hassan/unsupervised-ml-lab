...
| Duplicate rows | ⚠️ **52 found and removed** |
| Hidden missing values | ⚠️ `?` placeholder found in `workclass`, `occupation`, and `native-country` |
...
> 🗑️ `fnlwgt` was dropped — it is a census sampling weight with no predictive meaning for individual income.
...
Features after filtering:   16  ← nothing removed
```
...
| `my_utils` (local) | 🛠️ Custom profiling and plotting utilities |
